...
class SerachPersonidScreen extends StatefulWidget {
  SerachPersonidScreenState createState() => SerachPersonidScreenState();
}

bool Isshow = true;

class SerachPersonidScreenState extends State<SerachPersonidScreen> {
  List persons = [];
  List personsdf = [];
  List original = [];
  ...
  void search(String query) {
    if (query.isEmpty) {
      ...
      List result = [];
      original.forEach((p) {
        var personId = p["personId"].toString();
        if (personId.contains(query)) {
          result.add(p);
        }
      });
      persons = result;
      searchchk = persons.isEmpty;
    }
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: "ค้นหาเลขสมาชิก...",
          hintStyle: TextStyle(color: Colors.grey[500], fontSize: 16),
          prefixIcon: Icon(Icons.badge, color: Palette.kToDark.shade200),
          filled: true,
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.badge_outlined, size: 80, color: Colors.white54),
  ...
  Widget _buildMemberRow(Map person) {
    ...
    final pid = (person['personId'] ?? '').toString();
    return InkWell(
      onTap: () {
        Navigator.push(
          context,
